...
public class OrderCustomerCoProcessOnEventTimeFunction extends KeyedCoProcessFunction<Integer, Order, CustomerChangeLog, EnrichedOrder> {
    ...
    public void processElement1(Order order, Context ctx, Collector<EnrichedOrder> out) throws Exception {
        ...
        // try to enrich if there is no further customer event come within a certain period (e.g., 2m) in processing time
        ctx.timerService().registerProcessingTimeTimer(TimeUtils.timestampIn(120000));
    ...
    public void onTimer(long timestamp, OnTimerContext ctx, Collector<EnrichedOrder> out) throws Exception {
        if ( ctx.timeDomain() == TimeDomain.PROCESSING_TIME ) {
            // enrich with the latest customer info
            Long customerTimestamp = findLatestCustomerChangelogTimestamp();
            if ( customerTimestamp == null ) {
                // no customer events have been loaded yet, wait for 2m again
                ctx.timerService().registerProcessingTimeTimer(TimeUtils.timestampIn(120000));
            } else {
                enrichWithTheLatestCustomerInfo(customerTimestamp, out);
            }
    ...
    private void enrichWithTheLatestCustomerInfo(long customerTimestamp, Collector<EnrichedOrder> out) throws Exception {
        // enrich with the latest customer info
        Customer customer = customerMapState.get(customerTimestamp);
        Iterator<Map.Entry<Long,Order>> orderMapIterator = orderMapState.entries().iterator();
        while (orderMapIterator.hasNext()) {
            Map.Entry<Long,Order> entry = orderMapIterator.next();
            Order order = entry.getValue();
            out.collect(new EnrichedOrder(order, customer));
            orderMapIterator.remove();
        }
    }
    ...
    private Long findLatestCustomerChangelogTimestamp() throws Exception {
        TreeSet<Long> timestampSet = new TreeSet<>();
        customerMapState.keys().forEach(timestampSet::add);
        return timestampSet.size() == 0 ? null : timestampSet.last();
    }
